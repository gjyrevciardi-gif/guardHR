...
export default function DashboardPage() {
  ...
  return <AppShell>
    <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
      <div><p className="text-sm font-semibold text-teal">PËRMBLEDHJE</p><h1 className="mt-1 text-3xl font-bold">Mirë se erdhe, {user?.full_name || "HR"}</h1><p className="mt-2 text-slate-500">Shqyrto sesionet dhe sinjalet pa vendimmarrje automatike.</p></div>
      <Link href="/sessions/new" className="btn-primary"><Plus size={18}/> Sesion i ri</Link>
    </div>
    <div className="mt-8 grid gap-4 sm:grid-cols-3">
      <Metric icon={<CalendarDays/>} label="Gjithsej sesione" value={sessions.length}/>
    ...
    <section className="card mt-8 overflow-hidden">
      <div className="flex flex-col gap-4 border-b border-slate-200 p-5 sm:flex-row sm:items-center sm:justify-between">
        <div><h2 className="font-bold">Intervistat</h2><p className="text-sm text-slate-500">Lista e sesioneve të krijuara nga ju.</p></div>
        <div className="relative"><Search className="absolute left-3 top-3 text-slate-400" size={18}/><input className="input pl-10 sm:w-72" placeholder="Kërko kandidat…" value={query} onChange={e => setQuery(e.target.value)}/></div>
      </div>
      {loading ? <p className="p-8 text-slate-500">Duke ngarkuar…</p> : filtered.length === 0 ? <div className="p-12 text-center"><p className="font-semibold">Nuk ka sesione.</p><p className="mt-1 text-sm text-slate-500">Krijo sesionin e parë për të gjeneruar linkun e kandidatit.</p></div> :
    ...
    <section className="card mt-8 p-6">
      <div className="flex items-center gap-3"><span className="rounded-xl bg-slate-100 p-2 text-navy"><Settings2 size={20}/></span><div><h2 className="font-bold">Data retention</h2><p className="text-sm text-slate-500">Eventet e sesioneve të përfunduara mund të pastrohen pas kësaj periudhe. Video nuk ruhet në MVP.</p></div></div>
    ...
    <section className="card mt-8 p-6"><div className="flex items-center gap-3"><span className="rounded-xl bg-slate-100 p-2 text-navy"><History size={20}/></span><div><h2 className="font-bold">Audit log</h2><p className="text-sm text-slate-500">Veprimet më të fundit të llogarisë HR.</p></div></div><div className="mt-5 divide-y divide-slate-100">{auditLogs.slice(0, 8).map(log => <div key={log.id} className="flex flex-col justify-between gap-1 py-3 text-sm sm:flex-row"><span className="font-medium">{log.action}</span><span className="text-slate-500">{dateTime(log.created_at)} · {log.ip_address || "IP e panjohur"}</span></div>)}{auditLogs.length === 0 && <p className="py-4 text-sm text-slate-500">Nuk ka veprime të regjistruara.</p>}</div></section>
  </AppShell>;
}
